...
//!  Classe da estrutura de dado: Lista Duplamente Encadeada.
/*!  Implementação da estruturda de dado Lista Duplamente Encadeada,
 ...
 *  lista duplamente encadeada de tamanho zero,
...
T ListaDupla<T>::retiraDoInicioDuplo() {
	if (listaVazia()) {
		...
		head->getProximo() -> setAnterior(nullptr);
...
template<typename T>
bool ListaDupla<T>::igual(T dado1, T dado2) const {
	return dado1 == dado2;
}
...
template<typename T>
ElementoDuplo<T>* ListaDupla<T>::getHead() {
	return head;
}

template<typename T>
int ListaDupla<T>::getSize() {
	return size;
}
